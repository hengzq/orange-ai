...
public class AppController {
    ...
    @PostMapping
    public ApiResponse<String> add(@RequestBody @Validated AppCreateRequest request) {
        return ApiResponse.ok(appService.add(request));
    }

    @Operation(summary = "根据ID删除", operationId = "orange-ai:app:delete")
    @DeleteMapping("/{id}")
    public ApiResponse<Boolean> removeById(@PathVariable("id") String id) {
        return ApiResponse.ok(appService.removeById(id));
    }

    @Operation(summary = "根据ID更新", operationId = "orange-ai:app:update")
    @PutMapping("/{id}")
    public ApiResponse<Boolean> updateById(@PathVariable("id") String id, @RequestBody @Validated AppUpdateRequest request) {
        return ApiResponse.ok(appService.updateById(id, request));
    }

    @Operation(summary = "根据应用ID发布应用", operationId = "orange-ai:app:update")
    @PutMapping("/{id}/publish")
    public ApiResponse<Boolean> updatePublishById(@PathVariable("id") String id) {
        return ApiResponse.ok(appService.updatePublishById(id));
    }


    @Operation(summary = "根据ID查询最新数据", operationId = "orange-ai:app:get")
    @GetMapping("/{id}/latest")
    public ApiResponse<AppVO> getLatestById(@PathVariable("id") String id,
        ...
        return ApiResponse.ok(appService.getLatestById(id, latestReleased));
    }


    @Operation(summary = "分页查询", operationId = "orange-ai:app:page")
    @PostMapping(value = "/page")
    public ApiResponse<PageDTO<AppListResponse>> pageApps(@RequestBody AppPageRequest request) {
        PageDTO<AppListResponse> res = appService.pageApps(request);
        return ApiResponse.ok(res);
    }

    @PostMapping(value = "/list")
    @Operation(summary = "查询所有的数据", operationId = "orange-ai:app:list", description = "返回所有的数据")
    public ApiResponse<List<AppVO>> list(@RequestBody AppListParam query) {
    ...
    public Flux<ApiResponse<ConversationResponse>> conversationStream(@RequestBody AppConversationStreamParam param) {
